            ...
            if ($targetForInterfaces -and (Test-Path $genTool)) {
                try {
                    ...
                    if (Test-Path $localGenFile) {
                        ...
                        [System.IO.File]::WriteAllLines($localGenFile, $cleanLines)
...
"@
        ...
        if ($DLCs) {
            ...
            if ($dlcLines.Count -gt 0) {
                [System.IO.File]::WriteAllText((Join-Path $settingsDir "DLC.txt"), ($dlcLines -join "`r`n") + "`r`n")
            }
        }
        ...
        Write-Host "  [OK] Synchronized steam_settings/ in $baseDir (User: $($identity.Name), SteamID: $($identity.SteamId), Port: $finalListenPort)" -ForegroundColor Green
    }
...
"@
    ...
    $fwHelperBat = Join-Path $ExeDir "Configurar_Firewall_LAN.bat"
    $fwBatLines = @(
        '@echo off',
        'setlocal enabledelayedexpansion',
        'title ReFix - Configurador de Firewall LAN',
        '',
        'set "SCRIPT_DIR=%~dp0"',
        'if "!SCRIPT_DIR:~-1!"=="\" set "SCRIPT_DIR=!SCRIPT_DIR:~0,-1!"',
        '',
        'echo ====================================================================',
        'echo             ReFix - Configurador de Firewall LAN',
        'echo ====================================================================',
        'echo:',
        'echo Este script autoriza el juego y el puerto LAN en el Firewall',
        'echo de Windows para permitir partidas en red local desde Pendrive/USB.',
        'echo:',
        '',
        'net session >nul 2>&1',
        'if !errorlevel! neq 0 (',
        ...
        '    echo [OK] Reglas aplicadas para ejecutable: !EXE_NAME!',
        ')',
        '',
        ...
        'echo [OK] Puerto UDP !LAN_PORT! autorizado para descubrimiento LAN.',
        '',
        'echo:',
        'echo ====================================================================',
        'echo  Firewall configurado con exito. Listo para jugar en LAN!',
        'echo ====================================================================',
        'echo:',
    ...
    $logEntry = "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] [ReFix AutoDeploy] Deployed Mode: Re:Goldberg LAN sin Steam | Game: $GameName | AppID: $finalRealAppId | SteamID: $($identity.SteamId) | User: $($identity.Name) | Port: $finalListenPort | LocalSave: saves`r`n"
