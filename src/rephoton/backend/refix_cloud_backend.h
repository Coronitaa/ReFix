...
namespace ReFix::Photon::Backend {
    ...
    class ReFixCloudBackend : public IPhotonBackend,
                              public IPhotonAuthentication,
                              public IPhotonRegionProvider,
                              public IPhotonRoomService {
    public:
        explicit ReFixCloudBackend(const ReFixCloudConfig& config, std::shared_ptr<IPhotonTransport> transport = nullptr);
        virtual ~ReFixCloudBackend();
        ...
        BackendMode GetMode() const override { return BackendMode::ReFixCloud; }
        const char* GetName() const override { return "ReFixCloud"; }
        ...
        // Server-side simulation hook (used for local dev / loopback harness)
        void SetDirectRoomInstance(std::shared_ptr<Realtime::RoomState> room);
        std::shared_ptr<Realtime::RoomState> GetDirectRoomInstance() const { return m_room; }

    private:
        void HandleIncomingPacket(const std::vector<uint8_t>& packet);

        ReFixCloudConfig m_config;
