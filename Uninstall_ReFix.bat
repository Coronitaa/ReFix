...
title ReFix - Uninstall ^& Restore Tool
...
if "%TARGET_CHOICE%"=="1" (
    ...
    if exist "%BIN_DIR%select_folder.ps1" (
        for /f "usebackq delims=" %%I in (`powershell -NoProfile -ExecutionPolicy Bypass -File "%BIN_DIR%select_folder.ps1"`) do (
            set "TARGET_DIR=%%I"
        )
    ) else if exist "%SCRIPT_DIR%bin\select_folder.ps1" (
        for /f "usebackq delims=" %%I in (`powershell -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%bin\select_folder.ps1"`) do (
            set "TARGET_DIR=%%I"
        )
    )
